...
kMove::kMove(ToolHead* toolhead, const std::vector<double>& start_pos, const std::vector<double>& end_pos, double speed) {
    double velocity = 0.0; //std::min(speed, toolhead->getMaxVelocity());
    ...
    max_smoothed_v2 = 0.0;
    // smooth_delta_v2 = 2.0 * move_d * toolhead->getMaxAccelToDecel();
}
...
// kMove::kMove(Toolhead* th, const std::array<double, 4>& start, const std::array<double, 4>& end, double speed)
//     : toolhead(th), start_pos(start), end_pos(end) {
//     accel = toolhead->max_accel;
//     junction_deviation = toolhead->junction_deviation;
//     double velocity = std::min(speed, toolhead->max_accel);
...
//         axes_d[0] = axes_d[1] = axes_d[2] = 0.0;
//         move_d = std::abs(axes_d[3]);
//         double inv_move_d = move_d ? 1.0 / move_d : 0.0;
//         accel = 99999999.9;
//         velocity = speed;
//         is_kinematic_move = false;
//     } else {
...
// void kMove::limit_speed(double speed, double accel) {
//     double speed2 = speed * speed;
//     if (speed2 < max_cruise_v2) {
//         max_cruise_v2 = speed2;
//         min_move_t = move_d / speed;
...
// std::runtime_error kMove::move_error(const std::string& msg) {
//     std::string error_msg = msg + ": " +
//                             std::to_string(end_pos[0]) + " " +
//                             std::to_string(end_pos[1]) + " " +
//                             std::to_string(end_pos[2]) + " [" +
//                             std::to_string(end_pos[3]) + "]";
//     return std::runtime_error(error_msg);
// }

// void kMove::calc_junction(const kMove& prev_move) {
//     if (!is_kinematic_move || !prev_move.is_kinematic_move) {
//         return;
//     }
//     double extruder_v2 = toolhead->extruder_check_move(prev_move);
//     double junction_cos_theta = -(axes_r[0] * prev_move.axes_r[0] +
//                                   axes_r[1] * prev_move.axes_r[1] +
//                                   axes_r[2] * prev_move.axes_r[2]);
//     if (junction_cos_theta > 0.999999) {
//         return;
//     }
//     junction_cos_theta = std::max(junction_cos_theta, -0.999999);
...
//     double move_centripetal_v2 = 0.5 * move_d * tan_theta_d2 * accel;
//     double prev_move_centripetal_v2 = 0.5 * prev_move.move_d * tan_theta_d2 * prev_move.accel;
//     max_start_v2 = std::min({R_jd * junction_deviation * accel,
...
// void kMove::set_junction(double start_v2, double cruise_v2, double end_v2) {
//     double half_inv_accel = 0.5 / accel;
//     double accel_d = (cruise_v2 - start_v2) * half_inv_accel;
//     double decel_d = (cruise_v2 - end_v2) * half_inv_accel;
//     double cruise_d = move_d - accel_d - decel_d;
